...
//        })))
  ...
  def toJsonString(lockfile: Lockfile) = {
    toJson(lockfile).toJSONString()
  }

  def toJson(lockfile: Lockfile): JSONObject = {
    val obj = new JSONObject()
    obj.put("requirements", toJson(lockfile.requirements.asScala.toSet))
    obj.put("context", toJson(lockfile.context.asScala.toSet))
    obj.put("artifacts", toJson(lockfile.artifacts.asScala.toSet))
    obj
  }

  def toJson(requirements: Set[LockfileRequirement]) : java.util.Set[JSONObject] = {
...
//        (__ \ "exclusions").format[Set[String]])({
      obj.put("id", req.id)
      obj.put("constraints", req.constraints)
      obj.put("exclusions", req.exclusions)
      obj
    }.asJava
  }
  
  def toJson(contexts: Set[LockfileContext]) : java.util.Set[JSONObject] = {
...
//      })))
    ...
    contexts.map { context =>
      val obj = new JSONObject()
      obj.put("info", context.info)
      obj.put("id", context.id)
      obj.put("repository", context.repository)
      obj.put("locations", context.locations)
      obj.put("commit", context.commit)
      obj.put("hash", context.hash)
      obj
    }.asJava
  }

  def toJson(artifacts: Set[LockfileArtifact]) : java.util.Set[JSONObject] = {
    ...
    artifacts.map { artifact =>
      val obj = new JSONObject()
      obj.put("hash", artifact.hash)
      obj.put("size", artifact.size)
      obj.put("locations", artifact.locations)
      obj.put("attributes", artifact.attributes)
      obj.put("filename", artifact.filename)
      obj
    }.asJava
  }
}
